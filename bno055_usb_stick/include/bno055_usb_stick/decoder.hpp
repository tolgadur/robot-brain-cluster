...
public:
  ...
  static std_msgs::Float64 toHeadingAbsMsg(const bno055_usb_stick_msgs::Output &output) {
    static bool called=false;
    static double prev_heading;
    static double init_heading;
    static int full_turns=0;

    std_msgs::Float64 heading;
    heading.data = (output.euler_angles.heading) * (180.0/M_PI);
    return heading;
  }

  static std_msgs::Float64 toHeadingRelMsg(const bno055_usb_stick_msgs::Output &output) {
    static bool called=false;
    static double prev_heading;
    static double init_heading;
    static int full_turns=0;
    ...
    if(!called) {
      prev_heading=heading.data;
      init_heading=prev_heading;
    }

    if(called){
      if ((prev_heading > 270)&&(heading.data < 90)){
        full_turns = full_turns + 1;}

      if((prev_heading < 90) && (heading.data > 270)){
        full_turns = full_turns - 1;}
      }
    prev_heading = heading.data;
    heading.data = full_turns * 360 + heading.data;
    heading.data = heading.data - init_heading;
    called=true;
/*
    std::cout<<"heading"<<" "<<heading.data;
    std::cout<<"full_turns"<<" "<<full_turns;
*/
    return heading;
  }
